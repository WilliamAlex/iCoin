...
#import "ABNewPageFlowView.h"
#import "ABBannerSubView.h"


@interface ABNewPageFlowView()
...
@implementation ABNewPageFlowView
...
- (void)initialize{
    ...
    subviewClassName = @"ABBannerSubView";
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ABBannerSubView *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        ABBannerSubView *indexBannerSubiew = (ABBannerSubView *)cell;
        
        __weak __typeof(self) weakSelf = self;
        indexBannerSubiew.didSelectCellBlock = ^(NSInteger tag, ABBannerSubView *cell) {
...
- (void)reloadData
{
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[ABBannerSubView class]]) {
...
- (UIView *)dequeueReusableCell{
    ABBannerSubView *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(ABBannerSubView *)cell {
